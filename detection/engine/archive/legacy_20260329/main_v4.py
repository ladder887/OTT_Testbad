"""
Detection Engine V4 - Knowledge Graph Design (지식그래프 구성.md 기반)
...
class DetectionEngineV4:
    def __init__(self):
        ...
        logger.info("Detection Engine V4 Initialized")
    ...
    def _create_indexes(self):
        ...
        with self.driver.session() as session:
            indexes = [
                ...
                "CREATE INDEX content_path IF NOT EXISTS FOR (c:Content) ON (c.path)",
    ...
    def build_knowledge_graph(self, logs):
        ...
        with self.driver.session() as session:
            ...
            for log in logs:
                ...
                # Request ID 생성 (timestamp + client_ip + path 해시)
                ...
                if content_path and content_path != "-":
                    session.run(
                        """
                        MERGE (c:Content {path: $path})
                        ON CREATE SET
                            c.type = CASE
                                WHEN $path CONTAINS '.m3u8' THEN 'HLS'
                                WHEN $path CONTAINS '.mp4' THEN 'MP4'
                                ELSE 'unknown'
                            END,
    ...
    def detect_leeching(self):
        """
        리칭 패턴 탐지:
        - 동일 Token + Content를 여러 ClientIP/Domain이 사용
        """
        with self.driver.session() as session:
            result = session.run(
                """
                MATCH (ip:ClientIP)-[:MADE_REQUEST]->(r:Request)-[:USING_TOKEN]->(t:Token)
                MATCH (r)-[:FOR_CONTENT]->(c:Content)
                WITH t, c, count(DISTINCT ip) as ip_count, collect(DISTINCT ip.ip) as ips
                WHERE ip_count > 2
                RETURN t.value as token, c.path as content, ip_count, ips
    ...
    def run(self):
        """메인 루프"""
        logger.info("=" * 60)
        logger.info("Detection Engine V4 Started (Request-Centric)")
        logger.info("=" * 60)
            ...
            try:
                ...
                if suspicious:
                    ...
                    for item in suspicious:
                        logger.warning(f"  Token: {item['token']}, Content: {item['content']}")
                        logger.warning(f"  IP Count: {item['ip_count']}, IPs: {item['ips']}")
                else:
