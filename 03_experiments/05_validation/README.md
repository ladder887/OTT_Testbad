...
train만 끝난 단계에서는 test 전용 저강도 공격 variant를 요구하지 않도록 수집 단계도
명시한다. 전체 train/validation/test를 함께 감사할 때는 이 옵션을 생략한다.

```bash
python3 03_experiments/05_validation/audit_pilot_collection.py \
  --manifests 06_outputs/01_run_manifests/tnsm_100lc_20260827_main_v1 \
  --mode main \
  --required-splits train \
  --output 06_outputs/01_data_quality/main_train_audit.json
```
