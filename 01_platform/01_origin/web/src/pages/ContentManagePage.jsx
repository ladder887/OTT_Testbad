  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (err?.response?.status === 413) {
        setError(err?.response?.data?.error || '업로드 파일이 너무 큽니다. 파일 크기를 줄이거나 업로드 제한을 확인하세요.')
      } else if (err?.response?.status === 504) {
        setError('업로드 요청이 Edge에서 타임아웃(504) 되었습니다. 서버에서 변환이 계속될 수 있으니 잠시 후 목록을 새로고침하세요.')
      } else if (!err?.response) {
        setError('업로드 중 네트워크 연결이 중단되었습니다. Edge/API 컨테이너 상태와 업로드 제한을 확인하세요.')
      } else {
            ...
            {saving && sourceVideo && (
              <div className="upload-progress-wrap" aria-live="polite">
                <div className="upload-progress-head">
                  <strong>{uploadProgress < 100 ? `업로드 중 ${uploadProgress}%` : '업로드 완료, 서버에서 변환/등록 처리 중...'}</strong>
