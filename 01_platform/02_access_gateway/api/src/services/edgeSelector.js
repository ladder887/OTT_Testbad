...
function getEdgeById(rawEdgeId) {
  const edgeId = String(rawEdgeId || '').trim().toLowerCase()
  if (!edgeId) return null

  const region = Object.keys(REGION_EDGE_ID).find(
    (candidateRegion) => String(REGION_EDGE_ID[candidateRegion]).toLowerCase() === edgeId
  )
  if (!region) return null

  return {
    id: REGION_EDGE_ID[region],
    region,
    url: REGION_EDGE_URL[region] || DEFAULT_EDGE_URL,
  }
}
