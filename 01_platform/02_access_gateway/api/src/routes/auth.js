...
const loginLimiterWindowMs = Math.max(1000, Number(process.env.LOGIN_RATE_LIMIT_WINDOW_MS || (15 * 60 * 1000)));
const loginLimiterMax = Math.max(1, Number(process.env.LOGIN_RATE_LIMIT_MAX || 5));

// Rate limiting
const loginLimiter = rateLimit({
  windowMs: loginLimiterWindowMs,
  max: loginLimiterMax,
  message: '로그인 시도 횟수 제한을 초과했습니다. 잠시 후 다시 시도하세요.'
});
